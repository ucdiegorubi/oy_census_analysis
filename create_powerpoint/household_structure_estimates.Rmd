...
### Opportunity Youth are more likely than Connected Youth to be married
```{r}
...
analysis_data$foodstamps_per_household %>% 
  ...
  geom_errorbar(aes(ymin = percent, ymax = percent_upp), 
                position = position_dodge(width = .9), 
                width = .2) +
  geom_text(aes(label = scales::percent(percent, accuracy = .1), y = percent - .02), 
...
# household level
analysis_data$hht %>% 
  filter(oy_household_full != "All Other Household") %>% 
  mutate(HHT_label = fct_reorder(HHT_label, percent)) %>% 
  basic_plot_dodge() + 
  geom_errorbar(aes(ymin = !!error_bar_setting, ymax = percent_upp, group = HHT_label), 
                position = position_dodge(width = .9), 
                width = .2) +
  geom_text(aes(label = scales::percent(percent, suffix = "", accuracy = .1), group = HHT_label), 
            size = 2.7, 
            vjust = 1.25,
            position = position_dodge(width = .9)) +
