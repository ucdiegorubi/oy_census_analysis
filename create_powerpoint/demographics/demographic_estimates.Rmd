...
```{r}
age_population <- analysis_data$q1_demo$total_population_age_bracket %>% select(age_bracket, n) %>% spread(key = age_bracket, value = n) %>% unlist()
```
...
analysis_data$q1_demo$total_population_age_bracket %>% 
...
total_sample <- 
  ...
  analysis_data$q1_demo$total_oy_population %>% 
  select(oy_flag, n) %>% 
  arrange(oy_flag) %>% 
...
analysis_data$q1_demo$oy_race_alternate %>% 
...
analysis_data$q1_demo$oy_disability %>% 
...
```{r age-brackets}
analysis_data$q1_demo$oy_age_bracket %>% 
  filter(oy_flag != "Everyone Else", 
         age_bracket != "16-19") %>% 
  ggplot(aes(x = oy_flag, 
         y = percent)) + 
...
analysis_data$q1_demo$oy_race_gender %>% 
  ggplot(aes(x = oy_flag, 
             y = percent)) + 
...
analysis_data$q1_demo$oy_race_gender_alternate %>% 
  # filter(SEX_label != "Female") %>% 
...
analysis_data$q1_demo$geo_puma_pop %>% 
  ...
  geom_sf_text(label = 
                  analysis_data$q1_demo$geo_puma_pop$opp_youth, 
               size = 1.8) + 
  theme_void() + 
